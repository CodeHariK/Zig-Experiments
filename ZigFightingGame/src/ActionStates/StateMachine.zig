...
pub const CombatStateID = enum(u32) { Standing, Crouching, WalkingForward, WalkingBackwards, Jump, _ };
        ...
        if (self.Context) |context| {
            if (self.Registery.CombatStates[@intFromEnum(self.CurrentState)]) |State| {
                ...
                if (context.bTransition) {
                    ...
                    if (self.Registery.CombatStates[@intFromEnum(context.NextState)]) |NextState| {
...
fn TestOnUpdate(context: *CombatStateContext) void {
    const context_sub = @as(TestContext, @fieldParentPtr("base", context));
