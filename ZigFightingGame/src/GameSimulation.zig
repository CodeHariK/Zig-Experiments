...
var StandingCallbacks = StateMachine.CombatStateCallbacks{
    .OnUpdate = CommonStates.Standing.OnUpdate,
    .OnStart = CommonStates.Standing.OnStart,
    .OnEnd = CommonStates.Standing.OnEnd,
};

var WalkingForwardCallbacks = StateMachine.CombatStateCallbacks{
...
pub const GameState = struct {
    ...
    physicsComponents: ?[]Component.PhysicsComponent = null,

    // stateMachineComponents: [10]StateMachineComponent = [_]StateMachineComponent{.{}} ** 10,
    stateMachineComponents: ?[]StateMachineComponent = null,

    inputComponents: [2]InputComponent = [_]InputComponent{.{}} ** 2,

    pub fn Init(self: *GameState) void {
        for (self.physicsComponents) |*component| {
            component.* = Component.PhysicsComponent{};
        }
        for (self.stateMachineComponents) |*component| {
        ...
        self.stateMachineComponents[0].stateMachine.Registery.RegisterCommonState(
            StateMachine.CombatStateID.Standing,
            &StandingCallbacks,
        );
        self.stateMachineComponents[0].stateMachine.Registery.RegisterCommonState(
            StateMachine.CombatStateID.WalkingForward,
            &WalkingForwardCallbacks,
        );
    }
};
