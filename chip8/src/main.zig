...
const Chip8 = struct {
    ...
    registers: [15]u8, // 15 general purpose registers (V0 - VE)
    registerF: u8, // flag register
    ...
    pub fn init() Chip8 {
        var chip8 = Chip8{
            ...
            .program_len = 0,
            .registers = std.mem.zeroes([15]u8),
            .registerF = 0,
            .gfx = EMPTY_GFX,
    ...
    fn get_key(self: *Chip8) u8 {
        _ = self;
        // TODO: Implement blocking key wait.
        // For now, return 0 to allow compilation.
        return 0;
    }

    fn set_key(self: *Chip8, key: u8) void {
        self.key = EMPTY_KEY;
        self.key[key] = true;
    }
    ...
    fn decodeAndExecute(self: *Chip8) void {
        ...
        switch (code) {
            ...
            0xD => {
                ...
                // Draws a sprite at coordinate (Vx, Vy) that has a width of 8 pixels and a height of N (1-15) pixels. Each row of 8 pixels is read as bit-coded starting from memory location I; I value does not change after the execution of this instruction. As described above, VF is set to 1 if any screen pixels are flipped from set to unset when the sprite is drawn, and to 0 if that does not happen
                ...
                for (0..n) |yline| {
                    pixel = self.memory[self.index_register + yline];
                    for (0..8) |xline| {
                        if ((pixel & (@as(u8, 0x80) >> @intCast(xline))) != 0) {
                            const target_x = (@as(usize, start_x) + xline) % 64;
                            const target_y = (@as(usize, start_y) + yline) % 32;

                            if (self.gfx[target_y][target_x]) {
                                self.registerF = 1;
                            }
                            self.gfx[target_y][target_x] = !self.gfx[target_y][target_x];
                        }
                    }
                }
...
pub fn main() !void {
    ...
    while (!rl.windowShouldClose()) { // Detect window close button or ESC key
        ...
        const key = @as(u8, @intCast(@mod(@intFromEnum(rl.getKeyPressed()) - 65, 255)));
        if (key != -65) {
            if (key < 26 and KEY_MAP[key] < 12) {
                std.log.info("Key down: {}", .{KEY_MAP[key]});
                chip8.set_key(KEY_MAP[key]);
            }
        }
    }
}
