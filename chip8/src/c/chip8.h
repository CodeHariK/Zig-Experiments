 ...
 * apparently there are some computers where this is `0x600` (See
 * [wikipedia][]).
 */
#define PROG_OFFSET 512
 ...
 * ## Quirks
 *
 * The definitions below determine how the Quirks, as defined in Timendus'
 * [quirks-test][] are handled.
 *
 * Detailed explainations of the issues can be found in [langhoff][].
 *
 * Use the function `c8_set_quirks()` to a combination of the following flags
 * bitwise-OR'ed together:
 *
 * * `QUIRKS_VF_RESET` - Controls whether the AND/OR/XOR instructions reset the
 * VF register to 0.
 *   * The original COSMAC CHIP8 performed these directly in the ALU, so VF was
 *     always affected, but later implementations didn't.
 *   * See
 * [here](https://tobiasvl.github.io/blog/write-a-chip-8-emulator/#logical-and-arithmetic-instructions).
 * * `QUIRKS_MEM_CHIP8` - Controls whether the `I` register is incremented by
 * the Fx55 and Fx65 instructions.
 *   * The original COSMAC CHIP8 incremented the `I` register, but modern
 * implementations don't.
 *   * Don't use it if you're unsure, because that would be more compatible.
 *   * See
 * [here](https://tobiasvl.github.io/blog/write-a-chip-8-emulator/#fx55-and-fx65-store-and-load-memory).
 * * `QUIRKS_SHIFT` - If it is not set, `Vy` is copied into `Vx` when performing
 * the `8xy6` and `8xyE` shift instructions.
 ...
 *   * (Note to self: The INVADERS.ch8 game I found somewhere is an example of
 * where the shifting needs to be on)
 * * `QUIRKS_JUMP` - The CHIP-48 and SUPER-CHIP implementations originally
 * implemented the instruction as Bxnn as a jump to `Vx + xnn` rather than as
 * `V0 + xnn`
 *   * (it may have been a mistake).
 *   * See
 * [here](https://tobiasvl.github.io/blog/write-a-chip-8-emulator/#bnnn-jump-with-offset)
 * * `QUIRKS_CLIPPING` - With clipping enabled, sprites off the edge of the
 * screen are clipped. If it is disabled, then those sprites are wrapped to the
 * other side.
 *   * This is associated with XO-CHIP
 *
 * In addition, these constants are defined for convenience:
 *
 * * `QUIRKS_DEFAULT` - The default setting: It enables `VF Reset`, `Shift` and
 * `Clipping`.
 * * `QUIRKS_CHIP8` - Enables `VF Reset`, `Mem CHIP8`, `Disp Wait` and
 * `Clipping` for the best compatibility with the original CHIP8.
 * * `QUIRKS_SCHIP` - Enables `Clipping`, `Shift` and `Jump` for the best
 * compatibility with SUPER-CHIP.
 *
 * [langhoff]: https://tobiasvl.github.io/blog/write-a-chip-8-emulator/
 * [quirks-test]:
 * https://github.com/Timendus/chip8-test-suite/tree/main#quirks-test
 */
...
#define QUIRKS_CHIP8                                                           \
  (QUIRKS_VF_RESET | QUIRKS_MEM_CHIP8 | QUIRKS_DISP_WAIT | QUIRKS_CLIPPING)
 ...
 *
 * The **Fx0A** instruction is the one that waits for a specific key to be
 * pressed.
 */
int c8_waitkey();
...
/** ## Graphics
 * The _implementation_ should provide a platform specific way for the
 * interpreter core to draw its graphics.
 *
 */

/** `int c8_screen_updated();`  \
 * Returns true if the last instruction executed by `c8_step()` changed the
 * graphics, in which case the display should be updated.
 */
int c8_screen_updated();
 ...
 *
 * Returns 1 if the pixel is set - i.e. it should be drawn in the foreground
 * colour.
 *
 * Returns 0 if the pixel is cleared - i.e. it should be drawn in the background
 * colour.
 */
int c8_get_pixel(int x, int y);
