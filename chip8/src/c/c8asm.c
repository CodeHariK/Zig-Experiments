 ...
 * ## Overview
 *
 * The syntax is mostly based on that of [Cowgod's Chip-8 Technical Reference
 * v1.0][cowgod], by Thomas P. Greene
 *
 ...
 *
 * [^fake]: there aren't actual registers `K`, `F`, `B`, `HF` and `R` in the
 * interpreter, in but they're used in special syntax, particularly the `LD`
 * instruction.
 *
 ...
 *
 * * `Vx`, `Vy` and `Vn` refer to any of the 16 Chip8 registers, `V0` through
 * `VF`
 ...
 * |--------------------|-------------------|----------------------------------------------------|
 * |  0nnn              |   `sys nnn`       | System Call | |  00E0 |   `cls` |
 * Clear Screen                                       | |  00EE              |
 * `ret`           | Return                                             | | 1nnn
 * |   `jp addr`       | Jump to `addr`                                     | |
 * 2nnn              |   `call addr`     | Call routine at `addr` | |  3nkk |
 * `se Vn kk`      | Skip if `Vn` equals `kk`                           | | 4nkk
 * |   `sne Vn kk`     | Skip if `Vn` does not equal `kk`                   | |
 * 5xy0              |   `se Vx Vy`      | Skip if `Vn` equals `Vy` | |  6xkk |
 * `ld Vx, kk`     | Loads a literal value `kk` into `Vx`               | | 7nkk
 * |   `add Vn, kk`    | Add `kk` to register `Vx`                          | |
 * 8xy0              |   `ld Vx, Vy`     | Loads register `Vy` into `Vx` | |
 * 8xy1              |   `or Vx, Vy`     | Bitwise OR the value in `Vy` with
 * register `Vx`    | |  8xy2              |   `and Vx, Vy`    | Bitwise AND the
 * value in `Vy` with register `Vx`   | |  8xy3              |   `xor Vx, Vy` |
 * Bitwise XOR the value in `Vy` with register `Vx`   | |  8xy4              |
 * `add Vx, Vy`    | Add the value in `Vy` to register `Vx`             | | 8xy5
 * |   `xor Vx, Vy`    | Subtract the value in `Vy` from register `Vx`      | |
 * 8xy6              |   `shr Vx [, Vy]` | Shift `Vx` to the right with the
 * value in `Vy`     | |  8xy7              |   `subn Vx, Vy`   | Subtract the
 * value in `Vy` from `Vx`, no carry     | |  8xyE              |   `shl Vx [,
 * Vy]` | Shift `Vx` to the left with the value in `Vy`      | |  9xy0 |   `sne
 * Vx Vy`     | Skip if `Vn` does not equal `Vy`                   | |  Annn |
 * `ld I, nnn`     | Loads `nnn` into register `I`                      | | Bnnn
 ...
 * | |  Fx07              |   `ld Vx, DT`     | Loads the delay timer into
 * register `Vx`           | |  Fx15              |   `delay Vx`      | Loads
 * register `Vx` into the delay timer           | |  Fx18              | `sound
 * Vx`      | Loads register `Vx` into the sound timer           | |  Fx29 |
 * `hex Vx`        | Loads the 8&times;5 font sprite of `Vx` into `I`   | |
 * Fx30[^super]      |   `hexx Vx`       | Loads the 8&times;10 font sprite of
 * `Vx` into `I`  | |  Fx33              |   `bcd Vx`        | Load BCD value of
 * `Vx` into `I` to `I+2`           | |  Fx55              |   `stor Vx`       |
 * Stores `V0` through `Vx` to the address in `I`     | |  Fx65              |
 * `rstr Vx`       | Restores `V0` through `Vx` from the address in `I` | |
 * Fx75[^super]      |   `storx Vx`      | Stores `V0` through `Vx` to the
 * reserved space     | |  Fx85[^super]      |   `rstrx Vx`      | Restores `V0`
 * through `Vx` from the reserved space | |  00Cn[^super]      |   `scd n` |
 * Scroll down `n` pixels                             | |  00FB[^super]      |
 * `scr`           | Scroll right                                       | |
 * 00FC[^super]      |   `scl`           | Scroll Left | |  00FD[^super]      |
 * `exit`          | Exits the SuperChip48 interpreter                  | |
 * 00FE[^super]      |   `low`           | Low res mode | |  00FF[^super]      |
 * `high`          | Enable 128&times;64 high-res mode                  |
 *
...
static const char *inst_names[] = {
    "add",  "and",   "bcd",   "call", "cls",   "delay", "drw",  "exit", "hex",
    "hexx", "high",  "jp",    "key",  "ld",    "low",   "or",   "ret",  "rnd",
    "rstr", "rstrx", "scd",   "scl",  "scr",   "se",    "shl",  "shr",  "sknp",
    "skp",  "sne",   "sound", "stor", "storx", "sub",   "subn", "sys",  "xor",
};
...
static int apply_unary_op(const unsigned char op, const int val,
                          const int linenum) {
  switch (op) {
  case '+' | 0x80:
    return val;
...
static int apply_binary_op(const int l_op, const char op, const int r_op,
                           const int linenum) {
  ...
  *figures.stack = 0;
  ...
  while (*expression) {
    ...
    if (strchr(" \t\r", *expression)) {
      ...
      // if it's the first char I want to make sure it doesn't try to "bracket"
      // the initial 0
    ...
    } else if (is_unary_operator(expression) &&
               (!is_prev_figure || is_first_char_of_clause)) {
      if (is_first_char_of_clause)
        *(++operators.top) = '+';
      *(++operators.top) = ((unsigned char)*expression) | 0x80;
      ...
      is_first_char_of_clause = false;
               ...
               ((base = get_base(expression)) > 0)) {
      unsigned char operator_extended[2] = {*operators.top, *operators.top};
      while (figures.top >= figures.stack && is_prev_figure && prec > 0 &&
      ...
      if (base > 0) {
        ...
        *(++figures.top) = parse_int(&expression, linenum);
        ...
        if (*operators.top & 0x80) {
          (*figures.top) =
              apply_unary_op(*operators.top, *figures.top, linenum);
          operators.top--;
        }
      ...
      *bufptr = '\0';
      ...
      if (!success)
        exit_error("error:%d: Invalid Identifier %s in arithmetic expression\n",
                   linenum, buffer);
    }
  }
...
static inline void emit_w(const Stepper *stepper, uint16_t word) {
  const Emitted e = (Emitted){.type = ET_IMM16, .value = word};
  emit(stepper, e);
}
static inline void emit_e(const Stepper *stepper, uint16_t word,
                          size_t nibble_count) {
  const Emitted e = (Emitted){
      .type = 0b10000 | EXPRESSION_BITMASK | (nibble_count - 1), .value = word};
  emit(stepper, e);
}
...
scan_start:
  ...
  if (isalpha(*stepper->in)) {
    ...
    *tok = '\0';
    if (bsearch(stepper->token, inst_names,
                (sizeof inst_names) / (sizeof inst_names[0]),
                sizeof inst_names[0], inst_cmp)) {
      stepper->sym = SYM_INSTRUCTION;
    } else if (stepper->sym != SYM_INSTRUCTION) {
      if (stepper->token[0] == 'v' && isxdigit(stepper->token[1]) &&
          !stepper->token[2])
...
static int get_num(char *token, size_t nibble_count, const int linenum) {
  ...
  if (a < -(bound / 2) || a > (bound - 1)) {
    char format[128];
    sprintf(format,
            "error:%%d: number %%d takes more than %zd nibbles (%%0%zdX)\n",
...
int c8_assemble(const char *text) {
  ...
  for (int i = PROG_OFFSET; i < program.max_instr; i++) {
    ...
    if (program.bytes[i].type & EXPRESSION_BITMASK) {
      result = get_num(program.bytes[i].expression,
                       (program.bytes[i].type & BITNESS_BITMASK) + 1,
                       program.bytes[i].linenum);
    }
    if ((program.bytes[i].type != CONTINUED) &&
        (program.bytes[i].type & EMIT8_BITMASK)) {
  ...
  // Stupid Off by one
  if (program.max_instr < TOTAL_RAM &&
      program.bytes[program.max_instr].byte != 0) {
...
int c8_assemble_internal(Stepper *stepper) {
  ...
  while (stepper->sym != SYM_END) {
     ...
     * ```
     *
     * will cause the identifiers `sprite_x` and `sprite_y` to be associated
     * with the registers `V0` and `V1`
     *
     */
    case SYM_DEFINE: {
      char name[TOK_SIZE];
     ...
     *
     * Determines where in the program memory the next instruction will be
     * emitted
     *
     ...
     *
     * Writes all the bytes in the string, terminated with a null (`'\0'`)
     * character, to the output.
     *
     * For example `text "hello"` is equivalent to `db #68, #65, #6C, #6C, #6F,
     * #00`.
     */
    case SYM_TEXT: {
      nextsym(stepper);
    ...
    case SYM_INSTRUCTION:
      ...
      if (!strcmp("sys", stepper->token)) {
        ...
        const Emitted e = {.type = ET_EXP16, .value = 0x2000};
        emit(stepper, e);
        /**
         * ### RET - Return
         *
         ...
         * * `jp v0, nnn` - Jumps to the program location calculated from `v0 +
         * nnn` (`Bnnn`).
         */
      } else if (!strcmp("jp", stepper->token)) {
        nextsym(stepper);
        if (stepper->sym == SYM_IDENTIFIER || stepper->sym == SYM_NUMBER) {
          const Emitted e = {.type = ET_EXP16, .value = 0x1000};
          emit(stepper, e);
          ...
          expect(stepper, ',');
          if (stepper->sym == SYM_IDENTIFIER || stepper->sym == SYM_NUMBER) {
            const Emitted e = {.type = ET_EXP16, .value = 0xB000};
            emit(stepper, e);
          } else
         ...
         * Skip if equal:
         *
         * * `se Vn, kk` - skips the next instruction if the value in `Vn`
         * equals `kk` (`3nkk`)
         * * `se Vx, Vy` - skips the next instruction if the value in `Vx`
         * equals the value in `Vy` (`5xy0`)
         */
         ...
         * Load instructions:
         *
         * * `ld I, nnn` - loads the 12-bit literal value `nnn` into register
         * `I` (`Annn`).
         * * `ld Vx, kk` - loads the literal value `kk` into `Vx` (`6xkk`).
         * * `ld Vx, Vy` - loads the value in `Vy` into `Vx` (`8xy0`).
         * * `ld Vx, DT` - loads the value of the _Delay Timer_ into `Vx`
         * (`Fx07`).
         *
         * These variations are provided for compatibility with [Cowgod][]'s
         * syntax, but the alternative syntax should be preferred:
         *
         * * `ld DT, Vx` - loads the value in `Vx` into the _Delay Timer_
         * (`Fx15`). Use [`delay Vx`](#delay---delay) instead.
         * * `ld ST, Vx` - loads the value in `Vx` into the _Sound Timer_
         * (`Fx18`). Use [`sound Vx`](#sound---play-a-sound) instead.
         * * `ld F, Vx` - loads the location of the 8&times;5 font sprite for
         * `Vx` into `I` (`Fx29`). Use [`hex Vx `](#hex---prepare-a-hex-sprite)
         * instead.
         * * `ld B, Vx` - stores the BCD representation of `Vx` in the memory
         * locations `I`, `I+1` and `I+2` (`Fx33`). Use [`bcd
         * Vx`](#bcd---bcd-representation) instead.
         * * `ld [I], Vx` - saves `V0` through `Vx` to the memory addresses `I`
         * through `I+x` (`Fx55`). Use [`stor Vx`](#stor---store-registers)
         * instead.
         * * `ld Vx, [I]` - loads `V0` through `Vx` from the memory addresses
         * `I` through `I+x` (`Fx65`). Use [`rstr
         * Vx`](#rstr---restore-registers) instead.
         * * `ld Vx, K` - Waits for a keypress, then loads the key pressed into
         * `Vx` (`Fx0A`). Use [`key Vx`](#key---wait-for-key-press) instead.
         *
         * #### Super Chip48 LD additions
         *
         * These forms are also provided for compatibility with [Cowgod][]'s
         * syntax, but the alternative syntax should be preferred:
         *
         * * `ld HF, Vx` - loads the location of the larger 8&times;10 font
         * sprite for `Vx` into `I` (`Fx30`). Use [`hexx
         * Vx`](#hexx---prepare-a-large-hex-sprite) instead.
         * * `ld R, Vx` - saves `V0` through `Vx` to a special reserved memory
         * space `R`[^rpl]  (`Fx75`) Use [`storx
         * Vx`](#storx---store-registers-extended) instead.
         * * `ld Vx, R` - loads `V0` through `Vx` from the special reserved
         * memory space `R`[^rpl] (`Fx85`) Use [`rstrx
         * Vx`](#rstrx---restore-registers-extended) instead.
         *
         * [^rpl]: The original SUPER-CHIP stored these in the calculator on
         * which it ran's RPL registers
         */
        ...
        if (stepper->sym == SYM_I) {
          expect(stepper, ',');
          if (stepper->sym == SYM_IDENTIFIER || stepper->sym == SYM_NUMBER) {
            const Emitted e = {.type = ET_EXP16, .value = 0xA000};
            emit(stepper, e);
          } else
         ...
         *
         * `xor Vx, Vy` - biwise exclusive-OR of the value in `Vy` with `Vx`
         * (`8xy3`).
         *
         * The result is stored in `Vx`
         */
         ...
         * [More
         * information](https://tobiasvl.github.io/blog/write-a-chip-8-emulator/#8xy6-and-8xye-shift).
         */
         ...
         * ### RND - Random number
         *
         * `rnd Vn, kk` - generates a random number masked with `kk` and stores
         * it in `Vn` (`Cnkk`).
         *
         ...
         *
         * `skp Vn` - Skips the next instruction if the key identified by `Vn`
         * is pressed (`En9E`)
         */
         ...
         *
         * `sknp Vn` - Skips the next instruction if the key identified by `Vn`
         * is not pressed (`EnA1`)
         */
         ...
         *
         * `hex Vx` - loads the location of the 8&times;5 font sprite for the
         * hex value represented by `Vx` into `I` (`Fx29`).
         *
         */
         ...
         *
         * `bcd Vx` - stores the BCD representation of `Vx` in the memory
         ...
         *
         * `key Vx` - Waits for a keypress, then loads the key pressed into `Vx`
         * (`Fx0A`).
         */
         ...
         *
         * `stor Vx` - Stores registers `V0` through `Vx` to the memory
         * addresses `I` through `I+x` (`Fx55`).
         */
         ...
         *
         * `rstr Vx` - Restores registers `V0` through `Vx` from the memory
         * addresses `I` through `I+x` (`Fx65`).
         */
         ...
         *
         * `hexx Vx` - loads the location of the 8&times;10 font sprite of the
         * hex value represented by `Vx` into `I` (`Fx30`).
         *
         ...
         *
         * `storx Vx` - Saves registers `V0` through `Vx` to a special reserved
         * memory space `R`[^rpl]  (`Fx75`)
         *
         ...
         *
         * `rstrx Vx` - Restores registers `V0` through `Vx` from the special
         * reserved memory space `R`[^rpl] (`Fx85`)
         *
 ...
 * * The Octo project's [Chip8 reference
 * PDF](https://github.com/JohnEarnest/Octo/blob/gh-pages/docs/chip8ref.pdf) by
 * John Earnest.
 * * The Octo project's [Mastering
 * SuperChip](https://github.com/JohnEarnest/Octo/blob/gh-pages/docs/SuperChip.md)
 * reference.
