...
void c8_set_quirks(unsigned int q) { quirks = q; }

unsigned int c8_get_quirks() { return quirks; }
  ...
  case 0x0000:
    if (opcode == 0x00E0) {
      ...
      for (y = 0; y < row; y++) {
        for (x = col - 1; x > 0; x--) {
          pixels[y * col + x] =
              (pixels[y * col + x] << 4) | (pixels[y * col + x - 1] >> 4);
        }
        pixels[y * col] <<= 4;
      }
      screen_updated = 1;
...
int c8_waitkey() { return (c8_opcode(C8.PC) & 0xF0FF) == 0xF00A; }
...
uint16_t c8_get_pc() { return C8.PC; }
...
int c8_screen_updated() { return screen_updated; }
