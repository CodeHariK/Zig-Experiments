...
type PPU struct {
	Memory           // Memory interface for PPU VRAM mapping
	console *Console // Reference to the orchestrator struct

	Cycle    int    // 0-340 pixels per scanline
	ScanLine int    // 0-261 (0-239=visible, 240=post, 241-260=vblank, 261=pre)
	Frame    uint64 // Total frames rendered

	// Object Attribute Memory and Palettes
	paletteData   [32]byte
	...
	register byte // Used to simulate open bus behavior

	// NMI (Non-Maskable Interrupt) flags
	nmiOccurred bool
	nmiOutput   bool
	nmiPrevious bool
	nmiDelay    byte

	// Background temporary variables (fetched during cycles to render the next 8 pixels)
	nameTableByte      byte
	...
	// Sprite temporary variables (evaluated during previous scanline)
	spriteCount      int
	...
	// $2007 PPUDATA
	bufferedData byte // for buffered VRAM reads
}

// NewPPU creates a new instance of the PPU
...
func (ppu *PPU) tick() {
	if ppu.nmiDelay > 0 {
		...
		if ppu.nmiDelay == 0 && ppu.nmiOutput && ppu.nmiOccurred {
			// Notify the CPU that a NMI happened
